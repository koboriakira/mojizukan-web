...
describe("classifyWord", () => {
  it("NGワードは ng を返す", () => {
    const result = classifyWord({ word: "NGテスト", collected: [], seeded: [], ngList: ["NGテスト"] });
    expect(result.status).toBe("ng");
  });

  it("プリセット辞書の言葉は dict を返す", () => {
    const result = classifyWord({ word: "いぬ", collected: [], seeded: [], ngList: [] });
    expect(result.status).toBe("dict");
  });

  it("既に収集済みの言葉は dup を返す", () => {
    const result = classifyWord({ word: "らいおん", collected: ["らいおん"], seeded: [], ngList: [] });
    expect(result.status).toBe("dup");
  });

  it("仕込み済みの言葉は seeded を返す", () => {
    const result = classifyWord({ word: "らいおん", collected: [], seeded: ["らいおん"], ngList: [] });
    expect(result.status).toBe("seeded");
  });

  it("問題のない言葉は ok を返す", () => {
    const result = classifyWord({ word: "らいおん", collected: [], seeded: [], ngList: [] });
    expect(result.status).toBe("ok");
  });

  it("ng チェックは dict チェックより優先される", () => {
    const result = classifyWord({ word: "いぬ", collected: [], seeded: [], ngList: ["いぬ"] });
    expect(result.status).toBe("ng");
  });
});
...
describe("getRandomWords", () => {
  it("指定した数のワードを返す", () => {
    const words = getRandomWords({ n: 3, collected: [], seeded: [] });
  ...
  it("返すワードは仕込み済みリストに含まれない", () => {
    const seeded = HAKKEN_WORDS.slice(0, 30);
    const words = getRandomWords({ n: 3, collected: [], seeded });
    for (const word of words) {
      expect(seeded).not.toContain(word);
    }
  });

  it("利用可能なワードが足りない場合は最大数まで返す", () => {
    const collected = HAKKEN_WORDS.slice(0, 33);
    const words = getRandomWords({ n: 5, collected, seeded: [] });
